...
We're able to input our desired row and column. Notably, the game doesn't complain when our index is far outside the board; it just keeps playing. I play a few rounds and lose, obviously. 
...
-   **Coordinate Math:** Since the index is calculated as `(row-1)*3 + (col-1)`, you can input numbers outside 1-3. For example, what happens if you input a Row of 1 and a Column of 11? Or a Row of 0?
...
I then run `pwndbg > distance &board &winner`, which gives me an absurdly large number in hex. I convert it to a decimal number; let's call it N. 
...
`computer = 'O'` you say? Not for long, my friend. 
...
Can I use a negative index in an array? Surely. C doesn't even bother to check array bounds, and anyway, it's all arithmetic in there. 
...
...Then it segfaults. Maybe because it's my own version of chall, on my own computer, and it's trying to find a flag that doesn't exist. Anyway:
